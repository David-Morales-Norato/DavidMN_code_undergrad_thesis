...
class CLAS_PR(tf.keras.Model):
    def __init__(self, shape, num_classes, snapshots, wave_length, dx, distance, tipo_muestreo, normalize_measurements, float_dtype, complex_dtype, snr):
...
class CLAS_PR_BACK(tf.keras.Model):
    ...
    def get_init(self, input):
        ...
        muestras = self.ruido(muestras)
        caracteristicas = self.backpropagation_layer([muestras,mascara_usada,var_de_interes])
        return tf.unstack(caracteristicas, num=2, axis = -1)

class CLAS_PR_INIT(tf.keras.Model):
    def __init__(self, shape, num_classes, p, k_size, n_iterations, snapshots, wave_length, dx, distance, tipo_muestreo, normalize_measurements, float_dtype, complex_dtype, snr):
